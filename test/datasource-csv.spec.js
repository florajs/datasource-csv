...
describe('datasource-csv', () => {
    ...
    describe('csv parsing', () => {
        let ds;
        let request;
        const csv = ['id;name;birthday', '1;Alice;1979-12-31', '2;Bob;1977-06-11', '3;Horst;1990-01-02'].join('\n');
        ...
        describe('limit', () => {
            ...
            it('should implement "page"', async () => {
                ...
                ds.prepare(request);
                const results = await ds.process(request);
                expect(results.data).to.eql([{ id: '3', name: 'Horst', birthday: '1990-01-02' }]);
            });
        });

        describe('filter', () => {
            it('should work for "equal"', async () => {
                request.filter = [[{ attribute: 'id', value: '2', operator: 'equal' }]];
                ds.prepare(request);
                const results = await ds.process(request);
                expect(results).to.be.an('object');
                expect(results.data).to.be.an('array');
                expect(results.data).to.eql([{ id: '2', name: 'Bob', birthday: '1977-06-11' }]);
            });
